...
import ssm.client.Utils.FileUtils;
...
class AESCipherTest {
    ...
    void encrypt() throws IOException, CryptoException {
        File fileToEncrypt = FileUtils.getFile(FILE_TO_COMMIT_TXT);
        File encryptedFile = File.createTempFile("enc_", "tmp");
        File encryptedFileProof = FileUtils.getFile(FILE_TO_COMMIT_ENCRYPTED);
        try {
    ...
    void decrypr() throws IOException, CryptoException {
        File encryptedFile = FileUtils.getFile(FILE_TO_COMMIT_ENCRYPTED);
        try {
            SecretKey key = AESCipher.secretKeyFromBase64("+cRaRuaSK1/RObE9oEOm6Q==");
