...
class DatabasesSteps : En {
	...
	init {
		...
		When("I get the list of databases") {
			runBlocking {
				databases = CouchdbSsmQueriesFunctionImpl().couchdbDatabaseListQueryFunction(config).invoke(
		...
		When("I get the database for") { chaincode: Chaincode ->
			runBlocking {
				databases = CouchdbSsmQueriesFunctionImpl().couchdbDatabaseGetQueryFunction(config).invoke(
